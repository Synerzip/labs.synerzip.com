...
Tags: add, arabic, assign, associate, attach, attachment, attachments, audio, audios, author, auto, automatic, batch, brazilian, bulk, categories, category, change, column, control, custom post type, custom post types, custom taxonomies, custom taxonomy, date, dates, default, define, delete, detach, exchange, featured, featured image, featured images, filter, gallery, galleries, german, image, image size, images, mass, media, mime, multimedia, nextgen, pages, parent page, period, portuguese, post type, post types, posts, quick, random, rapid, remove, replace, rules, search, set, spanish, standard, tag, taxonomies, taxonomy, thumb, thumbnail, thumbnails, thumbs, time, unset, update, user, video, videos
...
1. **first content image** if available in the media library or on an external server or **first assigned image**
...
Quick Featured Images adds a new column called 'Featured Image' in posts lists. The additional column is sortable by the image ID. It shows the currently assigned **featured image of each post** and **action links to set, replace, edit and remove the featured image at each post**.
...
= 5.1.3 =
* Improved in Presets: Avoiding featured image of last post repeated in new post
...
= 5.1.1 =
* Fixed undefined variable in Presets
...
* Added in Presets: option for displaying random featured images at each page load
* Added in image column: action links to add, change and remove featured images at each post
* Improved in Presets: Random image selection
...
= 4.0 =
* Added in Presets: import function for rules from the free plugin Quick Featured Images if available
* Added in Presets: option for removing the first content image automatically after the featured image was set successfully
...
* Fixed in Presets: remove forgotten image from an image set if the image is deleted in the library
...
= 3.9 =
* Improved: Presets are working in the frontend, too, e.g. at creating a post with 3rd-party applications
...
= 3.8.1 =
* Fixed in 'Preset Images': Broken single image rule.
* Tested successfully with WordPress 4.4.2
* Published on 2016-02-04

= 3.8 =
* Added in 'Preset Images': Selection of multiple images to set one of them randomly as default featured image
...
* Fixed in 'Preset Images': Missing author match case
...
= 3.7.1 =
* Fixed in 'Preset Images': Double output of 'no featured image' in rules order list
* Fixed in 'Preset Images': Missing default values of rules order
* Published on 2015-11-28

= 3.7 =
* Added in 'Preset Images': User defined order of applying the types of rules
* Added in 'Preset Images': Option 'First content image' also takes image of an external server
...
* Fixed in 'Preset Images' and  'Set, replace, remove': Remove passwort part of temporary downloaded file name
...
* Added in 'Preset Images': Option to use the first attached image as featured image automatically
...
= 4.0 =
Added in Presets: import function and option for removing the first content image automatically, some improvemnts and fixes
...
= 3.9 =
Presets are working in the frontend, too; improved headline hierarchy on options page

= 3.8.1 =
Fixed broken single image rule

= 3.8 =
Added in 'Preset Images': Selection of multiple images to assign one of them randomly per post

= 3.7.1 =
In Presets: Fixed two warnings

= 3.7 =
In Presets: Considers first image from an external server and automated removal of rules after an image is removed in the library, some fixes
